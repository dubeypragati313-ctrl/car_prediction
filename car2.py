...
@st.cache_data
def load_and_preprocess_data(file_path="Cardetails.csv"):
    # Load dataset
...
def main():
    ...
    try:
        X, y = load_and_preprocess_data("Cardetails.csv")
        model, num_cols, cat_cols = train_model(X, y)
    except FileNotFoundError:
        st.error("Error: `Cardetails.csv` not found in the current directory. Please upload or ensure the file exists.")
        st.stop()
